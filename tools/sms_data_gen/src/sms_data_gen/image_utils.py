...
def extract_tiles(img: Image.Image, size = 8) -> tuple[int, int, list[Image.Image]]:
    width, height = img.size
    cols, rows = width // 8, height // 8
    return cols, rows, [_extract_tile(img, size, row, col) for row in range(rows) for col in range(cols)]
